...
(async()=>{
  ...
  // 강제 초기화: localStorage DB 삭제 (admin 계정/비번 admin으로 복구)
  try { localStorage.removeItem(DB_STORAGE_KEY); } catch(_) {}
  ...
  function ensureUsernameIndex() {
    // Always rebuild usernameIndex from scratch for reliability
    db.usernameIndex = {};
    Object.values(db.users).forEach(u => {
      if (u.username) {
        db.usernameIndex[u.username.toLowerCase()] = u.email;
      }
    });
  }
  ensureUsernameIndex();
  ...
  function isUsernameTaken(username) {
    ...
    ensureUsernameIndex();
    return !!db.usernameIndex[username.toLowerCase()];
  }
...
window.GSApp.loginId = async function(id, pw){
  ...
  if(db.usernameIndex[candidate]) email = db.usernameIndex[candidate];
  // 이메일 직접 입력도 허용
  if(!email && db.users[candidate]) email = candidate;
  // usernameIndex와 users 모두에서 못 찾으면 전체 users에서 username으로 검색
  if(!email){
    for(const u of Object.values(db.users)){
      if(u.username && u.username.toLowerCase() === candidate){
        email = u.email;
        break;
      }
    }
