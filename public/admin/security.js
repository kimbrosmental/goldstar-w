...
(function(){
  ...
  async function saveAdmins() {
    const res = await fetch('/api/admin/admins', {
      ...
      body: JSON.stringify({ data: window.encrypt(admins) })
    });
  }
  ...
  function render(){
    ensureDefaultAdmin();
    var html = `<h3>관리자 계정 관리 <button class=\"btn\" id=\"btnAddAdmin\">추가</button></h3>`;
  ...
  document.addEventListener('DOMContentLoaded', async ()=>{ await loadAdmins(); await loadIPRules(); render(); });
})();
