...
(function(){
  ...
  function render(){
    ...
    orders.forEach(function(o,i){
      html += `<tr>
        ...
        <td>${o.status||''}</td>
        <td>${o.memo||''}</td>
        <td><button class="dashboard-top-btn small list-btn" onclick="editOrder(${i})">수정</button> <button class="dashboard-top-btn small list-btn" onclick="deleteOrder(${i})">삭제</button> <button class="dashboard-top-btn small list-btn" onclick="changeStatusOrder(${i})">상태변경</button></td>
      </tr>`;
    });
    html += '</tbody></table>';
  ...
  function showAddOrder(){
    showOrderModal('주문 추가', {}, function(data){
      orders.push(Object.assign({}, data, { orderId:'O'+Date.now(), date:new Date().toISOString() }));
      saveOrders().then(render);
    });
  }
