...
(function(){
  ...
  async function saveUsers() {
    const res = await fetch('/api/admin/users', {
      ...
      body: JSON.stringify({ data: window.encrypt(users) })
    });
  }
  // 암호화된 회원 DB에서 회원 목록 불러오기 (로컬스토리지)
  async function loadLocalUsers(){
    if(window.GSApp && typeof GSApp.loadDB==='function' && typeof GSApp.getDB==='function'){
      await GSApp.loadDB();
  ...
  window.editUser = function(idx){
    ...
    showUserModal('회원 정보 수정', u, function(data){
      (async()=>{ users[idx] = {...u, ...data}; await saveUsers(); await reloadAndRender(); })();
    });
  };
  window.deleteUser = function(idx){
    if(confirm('정말 삭제하시겠습니까?')){
      (async()=>{ users.splice(idx,1); await saveUsers(); await reloadAndRender(); })();
    }
  };
  window.changePwUser = function(idx){
    const u = users[idx];
    showUserModal('비밀번호 변경', u, function(data){
      (async()=>{ users[idx].password = data.password; await saveUsers(); await reloadAndRender(); })();
    }, true);
  };
  function showAddUser(){
    showUserModal('회원 추가', {}, function(data){
      (async()=>{ users.push({...data, id:Date.now(), created:new Date().toISOString(), status:'active', orders:0}); await saveUsers(); await reloadAndRender(); })();
    });
  }
  ...
  window.approveUser = function(idx){
    ...
    saveUsers().then(reloadAndRender);
  };
  // 대시보드 회원가입 요청 알림 표시
  function renderDashboardAlert(){
    const dashboard = document.getElementById('view-dashboard');
    const pendingUsers = window.users ? window.users.filter(u=>u.status==='pending') : [];
    if(pendingUsers.length){
      dashboard.innerHTML = `<div style="background:#FFD70022;color:#b5942b;padding:18px 24px;border-radius:12px;margin-bottom:24px;font-size:1.15em;font-weight:bold;cursor:pointer;" onclick="window.gotoPendingUser()">회원가입 요청 ${pendingUsers.length}건 - 클릭하여 승인</div>` + dashboard.innerHTML;
    }
  }
