...
export async function onRequest({ request, env }) {
  const HDR_TEXT = { 'Access-Control-Allow-Origin':'*', 'Access-Control-Allow-Methods':'GET,POST,OPTIONS', 'Access-Control-Allow-Headers':'Content-Type', 'Content-Type':'text/plain; charset=utf-8' };
  if (request.method === 'OPTIONS') return new Response(null, { status:204, headers: HDR_TEXT });
  ...
  try {
    if (request.method === 'GET') {
      ...
      return new Response('Method Not Allowed', { status:405, headers: HDR_TEXT });
    }
  } catch { return new Response('요청 본문 오류', { status:200, headers: HDR_TEXT }); }

  if (!key) return new Response('아이디를 입력하세요.', { status:200, headers: HDR_TEXT });

  try {
    if (await env.USERS.get(key)) return new Response('이미 존재하는 아이디입니다.', { status:200, headers: HDR_TEXT });

    // also check admin list in SECURITY.adminid
    ...
    if (secVal) {
      ...
      if (admins && admins.find(a => String(a.username||'').toLowerCase().trim()===key)) {
        return new Response('이미 존재하는 아이디입니다.', { status:200, headers: HDR_TEXT });
      }
    }
    return new Response('사용 가능한 아이디입니다.', { status:200, headers: HDR_TEXT });
  } catch (e) {
    return new Response('서버 오류', { status:200, headers: HDR_TEXT });
  }
}
