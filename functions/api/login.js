...
// 관리자 계정은 SECURITY.adminid, 일반 회원은 USERS에서 조회
// 응답은 항상 200, ok:true/false 로 성공/실패 구분
...
async function verifyPassword(stored, input) {
  ...
  if (stored === input) return true; // plain legacy
  ...
  if (stored.startsWith("pbkdf2$")) {
    try {
      ...
      const bits = await crypto.subtle.deriveBits(
        {name:"PBKDF2", hash:"SHA-256", salt, iterations}, key, 32*8
      );
...
export async function onRequest({ request, env }) {
  ...
  try {
      ...
      if (secVal) {
        ...
        if (admins) {
          ...
          if (found) {
            ...
            if (!ok) {
              return new Response(JSON.stringify({ok:false,error:"비밀번호가 일치하지 않습니다."}), {status:200, headers:CORS});
            }
    ...
    } catch { /* ignore admin path errors */ }
