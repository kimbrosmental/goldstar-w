// patched login.js with admin support
...
// ---- Admin login patch ----
async function verifyPasswordPatch(stored, input) {
...
export async function onRequest(context) {
  ...
  try {
        ...
        if (admins) {
          ...
          if (found) {
            if (String(found.status||"active") !== "active") {
              return new Response(JSON.stringify({ok:false,error:"비활성된 계정입니다."}), {status:200, headers:CORS});
            }
            const ok = await verifyPasswordPatch(found.password, pw);
    ...
    // Fall back to original onRequest if exists
    if (typeof onRequestOriginal === "function") {
      return await onRequestOriginal(context);
    }
    return new Response(JSON.stringify({ok:false,error:"존재하지 않는 아이디입니다."}), {status:200, headers:CORS});
  } catch {
