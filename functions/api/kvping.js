// functions/api/kvping.js
// GET /api/kvping?ns=USERS  -> write/read/delete test on that namespace
...
export async function onRequest({ request, env }) {
  try {
    ...
    const kv = env[ns];
    if (!kv) return new Response(JSON.stringify({ ok: false, error: 'no_binding', ns }), { status: 400, headers: CORS });
